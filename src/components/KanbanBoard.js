...
const KanbanBoard = () => {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        ...
        // Ensure the data contains the 'tickets' array
        if (!data.tickets || !Array.isArray(data.tickets)) {
          throw new Error("Data does not contain a valid 'tickets' array");
        }
        setTickets(data.tickets); // Assuming `setTickets` is a state function to store tickets

        // Ensure the data contains the 'users' array
        if (!data.users || !Array.isArray(data.users)) {
          throw new Error("Data does not contain a valid 'users' array");
        }

        setUsers(data.users); // Assuming `setUsers` is a state function to store users
      } catch (error) {
  ...
  // Group and sort tickets based on selected criteria
  const groupTickets = () => {
    if (!users) return {}; // Check if users is undefined
    ...
    if (grouping === "status") {
      ...
      grouped = tickets.reduce((acc, ticket) => {
        const userId = ticket.userId || "Unassigned"; // Use userId from the ticket
        const userName = userMapping[userId] || "Unassigned"; // Get user name from mapping
      ...
      grouped = tickets.reduce((acc, ticket) => {
        const priority = ticket.priority !== undefined ? ticket.priority : 0; // Default priority 0 if undefined
  ...
  // Sort tickets based on user choice
        ...
        {grouping === "priority"
          ...
          : Object.keys(groupedTickets).map((group) => (
              <div className="kanban-column" key={group}>
                <div className="kanban-header">
                  <div className="kanban-left">
                    {/* Insert UserIcon with user's initials */}
                    <UserIcon name={group} />
                    <h3>{group}</h3>
